"""
* counter
* defaultdict
* ordereddict
* namedtuple
* deque
"""

# * counter
# from collections import Counter  # for python 2
#
# device_temperatures = [13.5, 14.0, 14.0, 14.5, 14.5, 14.5, 15.0, 16.0]
#
# temperature_counter = Counter(device_temperatures)
# print(temperature_counter[14.5])


# * defaultdict
# from collections import defaultdict
#
# coworkers = [('Rolf', 'Mit'), ('Jen', 'Oxford'), ('Rolf', 'Cambridge'), ('Charlie', 'Manchester')]
#
# alma_maters = defaultdict(list)
#
# for coworker, place in coworkers:
#     alma_maters[coworker].append(place)
#
# # alma_maters.default_factory = None  # remove default factory
# # alma_maters.default_factory = int
#
# print(alma_maters)
# print(alma_maters['Rolf'])  # ['Mit', 'Cambridge']
# print(alma_maters['Anne'])  # [] -> default value
#
#
# my_company = 'Teclado'
#
# coworkers = ['Jen', 'Li', 'Charlie', 'Rhys']
# other_coworkers = [('Rolf', 'Apple Inc.'), ('Anna', 'Google')]
#
# coworker_companies = defaultdict(lambda: my_company)  # default dict takes a function
#
# for person, company in other_coworkers:
#     coworker_companies[person] = company
#
# print(coworker_companies[coworkers[0]])  # Teclado
# print(coworker_companies['Rolf'])  # Apple Inc.


# * ordereddict
# from collections import OrderedDict
#
# o = OrderedDict()
# o['Rolf'] = 6
# o['Jose'] = 12
# o['Jen'] = 3
# print(o)  # OrderedDict([('Rolf', 6), ('Jose', 12), ('Jen', 3)])
#
# o.move_to_end('Rolf')
# print(o)  # OrderedDict([('Jose', 12), ('Jen', 3), ('Rolf', 6)])
#
# o.move_to_end('Jen', last=False)
# print(o)  # OrderedDict([('Jen', 3), ('Jose', 12), ('Rolf', 6)])
#
# o.popitem()
# print(o)  # OrderedDict([('Jen', 3), ('Jose', 12)])


# * namedtuple
# from collections import namedtuple
#
# Account = namedtuple('Account', ['name', 'balance'])
# account = Account('checking', 1850.90)
# print(account.name)  # checking
# print(account.balance)  # 1850.9
# print(account)  # Account(name='checking', balance=1850.9)
#
# # account = ('checking', 1850.90)
# # accountNamedTuple = Account._make(account)
# # accountNamedTuple = Account(*account)
#
# print(account._asdict())  # OrderedDict([('name', 'checking'), ('balance', 1850.9)])


# * deque
from collections import deque

friends = deque(('Rolf', 'Charlie', 'Jen', 'Anna'))
friends.append('Jose')
friends.appendleft('Anthony')
print(friends)  # deque(['Anthony', 'Rolf', 'Charlie', 'Jen', 'Anna', 'Jose'])

friends.pop()
friends.popleft()
print(friends)  # deque(['Rolf', 'Charlie', 'Jen', 'Anna'])
